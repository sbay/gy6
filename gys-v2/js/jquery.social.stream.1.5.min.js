...
/*
 *	Modified to allow fluid columns:
...
getSortData:{postDate:function($elem){return parseInt($elem.attr("rel"),10)}},sortBy:"postDate",resizable:false,masonry:{columnWidth: obj.width()/4}});$(window).smartresize(function(){obj.isotope({masonry:{columnWidth: obj.width()/4}});});})},addevents:function(obj,$a){var self=this,speed=this.o.speed;var $container=$(".stream",obj),filters={};$(".controls",obj).delegate("a","click",function(){var x=$(this).attr("class");switch(x){case "prev":self.pauseTimer();ticker($a,"prev",speed);break;case "next":self.pauseTimer();ticker($a,"next",speed);break;case "play":self.rotate(obj);$(".controls .play").removeClass("play").addClass("pause");
